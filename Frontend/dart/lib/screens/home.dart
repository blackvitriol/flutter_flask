...
class _MyHomePageState extends State<MyHomePage> {
  ...
  List<int> _image;
  // String _predicted_image;
  String _predicted_text;
  final picker = ImagePicker();
  Response response;
  var dio = Dio();
  final analytics = deviceDetails();
  ...
  Future uploadSelectedFile(page_controller) async {
    ...
    try {
      ...
      response = await dio.post(api_addr, data: {
        'type': 'ocr_request', 
        'image': base64Image, 
        'name': analytics.deviceName, 
        'ip': analytics.ip_address,
        });
      page_controller.nextPage(
          duration: Duration(seconds: 1), curve: Curves.easeIn);
      BotToast.showText(text: response.data['status'].toString());
      setState(() {
        _predicted_text = response.data['results'].toString();
        // _predicted_image = response.data['image'];
        // print(_predicted_image);
      });
    } catch (e) {
    ...
    return Container(
        ...
        child: Container(
            ...
            child: Column(
              ...
              children: [
                Text('Welcome to UrduReader  !', style: heading3),
                Text('UrduReader currently only works on cropped images of words.', style: heading5),
          ...
          child: Column(
            ...
            children: [
              ...
              Container(
                    height: SizeConfig.screenHeight * 0.3,
                    width: SizeConfig.screenWidth * 0.3,
                    child: _predicted_text == null
                    ? Center(child: Text('Select image on first page and press "Perform Actions".'))
                    : Text(_predicted_text, style: predUrdu_style)),
